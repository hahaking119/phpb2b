...
if (isset($_POST['do'])) {
	...
	if ($do == "apply") {
		...
		$online = $pdb->GetRow("SELECT name,if_online_support,config,description FROM {$tb_prefix}payments WHERE id='".intval($_POST['payment_id'])."'");
			...
			case 2:
				if(is_numeric($_POST['total_price'])){
					...
					if($last_order_id){
						...
						//Todo: for the service goods
						//$info['product'] = $pdb->GetOne("SELECT name FROM {$tb_prefix}goods WHERE id=".$_POST['product_id']);
						$info['product'] = L("add_charge", "tpl");
						$info['total_price'] = $total_price;
						$info['description'] = $info['body'] = L("add_charge", "tpl");
						$payment_api_file = API_PATH. "payments/".$online['name']."/api.php";
						if(!file_exists($payment_api_file)){
							flash();
						}
						require ($payment_api_file);
						$port = new Passport($info,$configs);
						if($online['name']=="alipay") $link =$port->create_url();
